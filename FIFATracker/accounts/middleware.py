...
class LastUserActivityMiddleware(object):
    ...
    def process_request(self, request):
        if request.user.is_authenticated:
            try:
                last_activity = datetime.strptime(request.session.get(
                    "last-activity"), '"%Y-%m-%d %H:%M:%S.%f"')
            ...
            request.session["last-activity"] = dumps(
                datetime.now(), default=self.json_serial)
