    ...
    try:
        ...
        if possible_potential:
            if len(possible_potential) == 1:
                result = "The potential of this player is {} or less.".format(
                    possible_potential[0])
            else:
                result = "The potential of this player is {}-{}".format(
                    possible_potential[0], possible_potential[-1])
        else:
...
def ajax_calcwage(request):
    try:
        ...
        if leagueid == 0:
            leagueid = DataUsersLeagueteamlinks.objects.for_user(
                current_user).filter(teamid=teamid).first().leagueid
        else:
            if "," in leagueid:
        ...
        team = DataUsersTeams.objects.for_user(
            current_user).filter(teamid=teamid).first()

        league = DataUsersLeagues.objects.for_user(
            current_user).filter(leagueid=leagueid).first()
        ...
        result = "Weekly wage of a {} yo, {} rated player playing for {} in {} league is {} {}".format(
            age, ovr, team.teamname, league.leaguename, wage, currency_symbols[currency])
    except Exception as e:
